...
import PersonImage from "@/app/assets/person.png"
export default function LeadersCardDesign({eachLeader,setShowDetails}){
    return(
        <div className=" w-[18rem] h-[24rem] group shrink-0 hover:rounded-r-none overflow-hidden  rounded-2xl border border-zinc-700 relative flex justify-center items-center">
            <Image src={eachLeader.image} alt="image" className=" object-cover h-full   "/>
             <div className=" absolute bottom-0 h-[47%]  w-full rounded-xl overflow-hidden group-hover:rounded-r-none  ">
                 ...
                 <div className=" w-full h-[7rem]  border-zinc-100 absolute bottom-0 left-0 flex flex-col items-center">
                     ...
                     <LuCircleFadingArrowUp className=" absolute -top-10 text-[2.1rem] rotate-90 group-hover:rotate-[270deg] duration-300 text-sky-500 right-2" onClick={()=>setShowDetails(true)}/>
                     </div>
             </div>
